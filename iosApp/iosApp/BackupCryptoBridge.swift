...
@objc(BackupCryptoBridge)
final class BackupCryptoBridge: NSObject {

    @objc(encryptWithPlaintext:password:saltBase64:iterations:error:)
    static func encrypt(
        plaintext: String,
        password: String,
        saltBase64: String,
        ...
        do {
            guard let plaintextData = plaintext.data(using: .utf8) else {
                throw NSError(
                    ...
                    code: 1001,
                    userInfo: [NSLocalizedDescriptionKey: "Plaintext encoding failed."]
                )
            }
            ...
            guard let saltData = Data(base64Encoded: saltBase64) else {
                throw NSError(
                    domain: "BackupCryptoBridge",
                    code: 1002,
                    userInfo: [NSLocalizedDescriptionKey: "Invalid salt."]
                )
            }
            ...
            errorOut?.pointee = wrapped
            return nil
        }
    }

    @objc(decryptWithCiphertextBase64:password:saltBase64:ivBase64:iterations:error:)
    static func decrypt(
        ...
        do {
            ...
            guard let ivData = Data(base64Encoded: ivBase64) else {
                throw NSError(
                    domain: "BackupCryptoBridge",
                    code: 2002,
                    userInfo: [NSLocalizedDescriptionKey: "Invalid IV."]
                )
            }
            ...
            guard let plaintext = String(data: opened, encoding: .utf8) else {
                throw NSError(
                    domain: "BackupCryptoBridge",
                    code: 2005,
                    userInfo: [NSLocalizedDescriptionKey: "Plaintext decoding failed."]
                )
            }
    ...
    private static func deriveKeyPBKDF2(
        ...
        guard result == kCCSuccess else {
            throw NSError(
                domain: "BackupCryptoBridge",
                code: 3001,
                userInfo: [NSLocalizedDescriptionKey: "PBKDF2 derivation failed."]
            )
        }
